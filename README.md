...
## สำคัญ: ต้องรัน SQL เพิ่มอีกครั้ง (ครั้งที่ 3)
เฟสนี้เพิ่มคอลัมน์ `pre_test` และ `post_test` ในตาราง `students` (เพื่อให้ Teacher Dashboard เห็นภาพรวม Pre/Post ทั้งห้องได้) — กลับไปรัน `supabase/schema.sql` ทั้งไฟล์อีกครั้งใน SQL Editor (ตอนนี้ปลอดภัยต่อการรันซ้ำทั้งไฟล์แล้ว ไม่ error เหมือนก่อนหน้า)
